...
- **Getting GLMMs from R**. You will receive a gentle reminder in the form of an error. Note that even once this is supported, you're subject to the constraints above. Moroever, the various `quasi` families in R are not supported because the corresponding distributions aren't in Distributions.jl.
