...
- **advanced models in either language** ~(e.g., `zerocorr!`, `fulldummy` in Julia or `||` in R, which are not completely synonymous anyway).~ `fulldummy` in Julia and `||` in R should work. `zerocorr` will work for continuous variables and factors with two levels, but won't work with factors with more than two levels. There's not really a trivial way to deal with this at the moment, sorry. <!-- `fulldummy`, `dummy` `lmer_alt`-- > Posthoc `zerocorr!` is not detected. Use `zerocorr` in your formula instead.
...
- **complicated formulae syntax in R**, which is really just the same as above, but also includes things like `I()` or `(x + y + z)^2` to represent "all main effects and two-way interactions".
